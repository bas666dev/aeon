...
from aeon_core.glyphs import get_glyph_phase, list_glyph_octave
...
@app.get("/glyphs", response_model=List[GlyphPhase], tags=["glyphs"])
def glyph_octave(phase: Optional[int] = Query(None, ge=0, le=7)) -> List[GlyphPhase]:
    if phase is None:
        payload = list_glyph_octave()
    else:
        glyph = get_glyph_phase(phase)
        if not glyph:
            raise HTTPException(status_code=404, detail="Glyph phase not found")
        payload = [glyph]
    return [GlyphPhase.parse_obj(item) for item in payload]
